...
const siteUrl=()=>String(process.env.SITE_URL||'https://kronika82.vercel.app').replace(/\/$/,'');
...
function renderNewsletter({article,recipient,mode='article'}){
  ...
  const text = isUpdate ? (article.newsletter_update_excerpt || article.newsletter_teaser || excerpt(article.summary, article.content)) : article.newsletter_teaser || excerpt(article.summary, article.content);
  const label=isUpdate?'AKTUALIZACJA':'NOWY ARTYKUŁ';
  const subject=`${isUpdate?'AKTUALIZACJA: ':''}${cleanText(article.title)} • Kronika 82`;
  const preheader=isUpdate?`Aktualizacja: ${text}`:text;
...
<tr><td style="padding:23px 28px 10px"><p style="margin:0;font-size:17px;line-height:1.65">${esc(text)}</p></td></tr>
...
<tr><td style="padding:18px 28px;border-top:1px solid #b8afa0;color:#6e685e;font:11px/1.5 Arial,sans-serif;text-align:center">Newsletter Kroniki 82.<br><a href="${esc(removeUrl)}" style="color:#6e685e;text-decoration:underline">Wypisz się z newslettera</a></td></tr>
  ...
  const textBody=['KRONIKA 82',label,'',cleanText(article.title),'',cleanText(article.summary),'',text,'',`Czytaj dalej: ${articleUrl}`,'',`Wypisz się: ${removeUrl}`].join('\n');
  return {subject,html,text:textBody,articleUrl,removeUrl};
}
