...
function renderNewsletter({article,recipient,mode='article'}){
  ...
  const teaser=cleanText(isUpdate?article.newsletter_update_excerpt:article.newsletter_teaser)||excerpt(article.summary,article.content);
  const label=isUpdate?'AKTUALIZACJA':'NOWY ARTYKUŁ';
  const subject=`${isUpdate?'AKTUALIZACJA: ':''}${cleanText(article.title)} • The 82 Chronicle`;
...
<tr><td style="padding:28px 28px 18px;text-align:center;border-bottom:1px solid #24211d"><div style="font-size:42px;font-weight:700;line-height:1;letter-spacing:-2px">The 82 Chronicle</div><div style="margin-top:9px;color:#6e685e;font:11px Arial,sans-serif;letter-spacing:1.5px;text-transform:uppercase">Wiadomości spod numeru 82</div></td></tr>
...
<tr><td style="padding:18px 28px;border-top:1px solid #b8afa0;color:#6e685e;font:11px/1.5 Arial,sans-serif;text-align:center">Rodzinny newsletter The 82 Chronicle.<br><a href="${esc(removeUrl)}" style="color:#6e685e;text-decoration:underline">Wypisz się z newslettera</a></td></tr>
